...
- `npm run test:commit-msg` runs the hook's own suite: the cases, the messages that must *not* be rejected, a scissors matrix across cleanup modes and comment characters, and a fuzz pass. CI runs it before it checks any message
